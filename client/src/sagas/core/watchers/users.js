...
export default function* usersWatchers() {
  yield all([
    ...
    takeEvery(EntryActionTypes.CARD_MEMBERSHIP_IN_CURRENT_CARD_CREATE, ({ payload: { data } }) =>
      services.createCardMembershipInCurrentCard(data),
    ),
    takeEvery(EntryActionTypes.CARD_MEMBERSHIP_IN_CURRENT_CARD_DELETE, ({ payload: { userId } }) =>
      services.deleteCardMembershipInCurrentCard(userId),
    ),
  ]);
}
