...
export function* createCardMembershipInCurrentCard(data) {
  const { cardId } = yield select(selectors.selectPath);

  yield put(actions.createCardMembership(cardId, data));

  let cardMembership;
  try {
    ({ item: cardMembership } = yield call(request, api.createCardMembership, cardId, data));
  } catch (error) {
    console.error('[Saga] CardMembership create failed:', error);
    yield put(actions.createCardMembership.failure(cardId, error));
    return;
  }

  yield put(actions.createCardMembership.success(cardMembership));
}

export function* deleteCardMembershipInCurrentCard(userId) {
  const { cardId } = yield select(selectors.selectPath);

  let deletedCardMembership;
  try {
    ({ item: deletedCardMembership } = yield call(
      request,
    ...
    console.error('[Saga] Delete failed:', error);
    return;
  }

  yield put(actions.handleCardMembershipDelete(deletedCardMembership));
}
