...
export default class extends BaseModel {
  ...
  getBoardsModelArrayAvailableForUser(userModel) {
    ...
    const allBoards = this.getBoardsQuerySet().toModelArray();

    const availableBoards = allBoards.filter((boardModel) => {
      const isAvailable = boardModel.isAvailableForUser(userModel);
      return isAvailable;
    });
  ...
  isAvailableForUser(userModel) {
    ...
    // Board membership
    if (this.hasMembershipWithUserIdInAnyBoard(userModel.id)) {
      return true;
    }

    // Card membership - user has access to any card in this project
    const hasCardAccess = userModel.cardMemberships
      .toModelArray()
      .some((cm) => {
        if (!cm.card || !cm.card.board) {
          return false;
        }
        return cm.card.board.projectId === this.id;
      });
    ...
    // List membership - user has access to any list in this project
    const hasListAccess = userModel.listMemberships
      .toModelArray()
      .some((lm) => {
        if (!lm.list || !lm.list.board) {
          return false;
        }
        return lm.list.board.projectId === this.id;
      });
