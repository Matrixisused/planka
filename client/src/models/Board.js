...
export default class extends BaseModel {
  ...
  isAvailableForUser(userModel) {
    ...
    if (this.hasMembershipWithUserId(userModel.id)) {
      return true;
    }

    // Check project access (manager/admin)
    if (this.project.isExternalAccessibleForUser(userModel)) {
      return true;
    }

    // Check card memberships - user has access to any card in this board
    const cardMemberships = userModel.cardMemberships.toModelArray();

    const hasCardAccess = cardMemberships.some((cm) => {
      const hasCard = !!cm.card;
      const matchesBoardId = cm.card && cm.card.boardId === this.id;
      return hasCard && matchesBoardId;
    });
    ...
    // Check list memberships - user has access to any list in this board
    const listMemberships = userModel.listMemberships.toModelArray();

    const hasListAccess = listMemberships.some((lm) => lm.list && lm.list.boardId === this.id);
