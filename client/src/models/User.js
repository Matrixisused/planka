...
export default class extends BaseModel {
  ...
  getMembershipProjectsModelArray() {
    ...
    const boardMembershipsArray = this.getBoardMembershipsQuerySet().toModelArray();

    const projectsFromBoardMemberships = boardMembershipsArray
    ...
    // Also include projects from CardMemberships
    const cardMembershipsArray = this.cardMemberships.toModelArray();

    const projectsFromCardMemberships = cardMembershipsArray
      .flatMap(({ card }) => {
        if (!card || !card.board || !card.board.project) {
          return [];
        }

        const projectModel = card.board.project;
    ...
    // Also include projects from ListMemberships
    const listMembershipsArray = this.listMemberships.toModelArray();

    const projectsFromListMemberships = listMembershipsArray
      .flatMap(({ list }) => {
        if (!list || !list.board || !list.board.project) {
          return [];
        }

        const projectModel = list.board.project;
        if (projectIds.includes(projectModel.id)) {
          return [];
        }
    ...
    const result = [...projectsFromBoardMemberships, ...projectsFromCardMemberships, ...projectsFromListMemberships];
    return result;
  }
