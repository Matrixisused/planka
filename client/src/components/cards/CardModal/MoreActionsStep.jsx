...
const MoreActionsStep = React.memo(({ onClose }) => {
  ...
  const { canEditType, canDuplicate, canMove, canCreatePublicLink } = useSelector((state) => {
  ...
  const handlePublicLinkClick = useCallback(() => {
    dispatch(entryActions.openPublicLinkModal(null, null, card.id));
    onClose();
  }, [card.id, dispatch, onClose]);
