...
/**
 * @swagger
 * /users/me/mentioned-cards:
 *   get:
 *     summary: Get cards where current user is mentioned
 *     description: Retrieves all cards where the current user is mentioned in description or comments, sorted by priority (desc) and dueDate (asc)
 *     tags:
 *       - Cards
 *     operationId: getMentionedCards
 *     parameters:
 *       - name: before
 *         in: query
 *         required: false
...
module.exports = {
  ...
  async fn(inputs) {
    sails.log.info('[mentioned-cards] Controller called', {
      url: this.req.url,
      isSocket: this.req.isSocket,
      hasCurrentUser: !!this.req.currentUser,
    ...
    if (!currentUser) {
      sails.log.warn('[mentioned-cards] No currentUser, throwing unauthorized');
      throw 'unauthorized';
    }

    sails.log.info('[mentioned-cards] Fetching cards for user', {
      userId: currentUser.id,
      before: inputs.before,
      limit: inputs.limit || 50,
    });

    const cards = await Card.qm.getByMentionedUserId(currentUser.id, {
      before: inputs.before,
      limit: inputs.limit || 50,
    });

    sails.log.info('[mentioned-cards] Cards found', { count: cards.length });
