...
module.exports = {
  ...
  async fn(inputs) {
    ...
    if (!boardMembership) {
      ...
      cardMembership = await sails.helpers.users.getCardMembership(currentUser.id, card.id);

      if (!cardMembership) {
        throw Errors.CARD_NOT_FOUND; // Forbidden
      }

      isEditor = cardMembership.role === CardMembership.Roles.EDITOR;
    } else {
      isEditor = boardMembership.role === BoardMembership.Roles.EDITOR;
    }
