  ...
  async fn(inputs) {
    ...
    if (currentUser.role !== User.Roles.ADMIN || project.ownerProjectManagerId) {
      ...
      if (!isProjectManager) {
        ...
        if (!boardMembership) {
          // Check if user has access to any cards or lists in this board
          const lists = await List.qm.getByBoardId(board.id);
          const listIds = sails.helpers.utils.mapRecords(lists);

          const listMemberships = await ListMembership.qm.getByListIds(listIds);
          const hasListAccess = listMemberships.some(lm => lm.userId === currentUser.id);
          ...
          if (!hasListAccess) {
            ...
            const hasCardAccess = userCardMemberships.length > 0;
    ...
    if (!boardMembership && currentUser.role !== User.Roles.ADMIN) {
      const isProjectManager = await sails.helpers.users.isProjectManager(
        currentUser.id,
        project.id,
      );
      ...
      if (!isProjectManager) {
        // User has no board membership, filter to only accessible cards
        const allCardIds = sails.helpers.utils.mapRecords(cards);

        // Get user's card memberships
        const userCardMemberships = await CardMembership.qm.getByCardIds(allCardIds);
        ...
        const userListMemberships = await ListMembership.qm.getByListIds(finiteListIds);
        const accessibleListIds = new Set(
          userListMemberships
            .filter(lm => lm.userId === currentUser.id)
            .map(lm => lm.listId)
        );

        // Filter cards: show only cards with direct access or cards in accessible lists
        cards = cards.filter(card =>
          accessibleCardIds.has(card.id) || accessibleListIds.has(card.listId)
        );
      }
    }
    ...
    const cardMemberships = await CardMembership.qm.getByCardIds(cardIds);
    const listMemberships = await ListMembership.qm.getByListIds(finiteListIds);
